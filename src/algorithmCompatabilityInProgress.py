...
path = "/home/helio/Group-4-Project/data/domo/" #make a folder and put all json files u want to use in it.
...
for j in range(numRows):
    ...
    currentCellTime = pd.to_datetime(currentCellTime)
    prevCellTime = pd.to_datetime(prevCellTime)
...
x_to_pred = np.array([np.ravel(X1), np.ravel(X2)])
plt.contourf(X1, X2, classifier.predict(x_to_pred).reshape(X1.shape),
